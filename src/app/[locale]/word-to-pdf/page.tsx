    ...
    <main className="mx-auto w-full max-w-6xl px-4 pb-32 pt-5 md:pb-36 md:pt-6">
      <section className="mb-4 rounded-lg border bg-card px-4 py-3 shadow-sm md:px-5">
        <nav className="mb-3 flex items-center gap-2 text-sm text-muted-foreground" aria-label="Breadcrumb">
          ...
          <span className="font-medium text-foreground">{t(safeLocale, "wordToPdfTitle")}</span>
        </nav>
        <h1 className="text-2xl font-bold leading-tight tracking-normal text-card-foreground md:text-3xl">{t(safeLocale, "wordToPdfTitle")}</h1>
        <p className="mt-1 max-w-2xl text-sm leading-6 text-muted-foreground">{t(safeLocale, "wordToPdfDescription")}</p>
      </section>
      ...
      <section className="mt-5 rounded-lg border bg-card p-6 md:p-8">
        <h2 className="text-2xl font-bold text-card-foreground">{t(safeLocale, "wordToPdfAboutTitle")}</h2>
        <p className="mt-3 max-w-3xl text-sm leading-6 text-muted-foreground">{t(safeLocale, "wordToPdfSeoBody")}</p>
      </section>
    </main>
  );
}
